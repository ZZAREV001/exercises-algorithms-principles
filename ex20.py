...
# import data we need here mlexpertdata
...
def get_k_means(user_feature_map, num_features_per_user, k):
    random.seed(42)
    initial_centroid_users = random.sample(sorted(
        list(user_feature_map.keys())), k)
    centroids = [Centroid(user_feature_map[initial_centroid_users])
...
def get_centroid_average(centroid, user_feature_map, num_features_per_user):
    ...
    return [centroid_dimension / len(centroid.closest_users)
